...
fis = readfis('my_controller.fis');                            % reading fuzzy system from file 
V_final = [];
fuel_final = [];
last_step = [];
history = [];
for i=1:number_of_initial_states                         % loop by initial states
   ...
   title(strcat('lander control (v.8) for initial atate: ',num2str(initial_states(i,:))));
   ...
   if if_animation
      ...
      ylabel('height [m]');
      title(strcat('Lander animation v.8'));
      xkab = [-3 -1 -4 -3 3 4 1 3];
      ykab = [0 2 2 8 8 2 2 0]*max_x/100;
      xciag = [-1 -2 2 1];
